        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xl sm:text-2xl font-bold">Royal Wedding Hall</span>
            </div>
            ...
            <div className="flex space-x-3">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-[var(--gold)] transition-all duration-300"
                >
                  <span className="text-white font-bold text-sm">{social.label[0]}</span>
                </a>
              ))}
            </div>
          </div>
            ...
            <ul className="space-y-2 sm:space-y-3">
              {footerLinks.quickLinks.map((link) => (
                <li key={link.href}>
                  <a
                    href={link.href}
                    className="text-gray-300 hover:text-[var(--gold)] transition-colors duration-300 text-sm sm:text-base"
                  >
                    {link.label}
                  </a>
                </li>
            ...
            <ul className="space-y-3 sm:space-y-4">
              {contactInfo.map((item) => (
                <li key={item.value}>
                  <a
                    href={item.href}
                    className="flex items-center space-x-3 text-gray-300 hover:text-[var(--gold)] transition-colors duration-300"
                  >
